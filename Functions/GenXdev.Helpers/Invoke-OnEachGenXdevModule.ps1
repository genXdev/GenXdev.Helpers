...
function Invoke-OnEachGenXdevModule {
    ...
    begin {
        ...
        $allGenXdevModules = @(GenXdev.FileSystem\Find-Item "$modulesPath\GenXdev*\3.3.2026\GenXdev*.psm1" -PassThru -NoRecurse | Microsoft.PowerShell.Core\ForEach-Object { [IO.Path]::GetFileNameWithoutExtension($_.FullName) }) +
            @("GenXdev.Scripts")
  ...
  process {
        ...
        foreach ($nextModule in $uniqueModulesNames) {
            ...
            function processModule {
                ...
                $licenseFilePath = (
                    "$($ModuleObj.FullName)\3.3.2026\LICENSE")

                # construct expected readme file path for published modules
                $readmeFilePath = (
                    "$($ModuleObj.FullName)\3.3.2026\README.md")
